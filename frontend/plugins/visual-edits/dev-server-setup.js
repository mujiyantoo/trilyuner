...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all 78islandtea.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*78islandtea\.sh$/)) {
        return true;
      }

      // Allow all 78islandtea.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*78islandtea\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@78islandtea.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@78islandtea.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
